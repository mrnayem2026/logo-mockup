...
function App() {
  const [selectedLogo, setSelectedLogo] = useState<JSX.Element | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const handleItemClick = (svgLogo: JSX.Element, id: string) => {
      ...
      <div className="svgItemContainer">
        {svgItems.map((item) => (
          <div
            ...
            onClick={() => handleItemClick(item.content, item.id)}
          >
            {item.content}
          </div>
        ))}
      ...
      <div className="logoBuilderSlideTemplateViewContainer">
        <div className="logoBuilderSlideTemplateViewItem1">
          ...
          <div className={`${selectedLogo ? "tshirt" : ""}`}>
            <div className="logoBuilderSlideSidebarItem ">{selectedLogo}</div>
          </div>
        </div>
